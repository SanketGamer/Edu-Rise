...
import SheetSection from '../../components/students/SheetSection'
 ...
 function SaveQuestionProgess(lecture){
  ...
  return courseData? (
    ...
    <div className="w-full">
      ...
      strokeWidth={4} trailWidth={4} strokeColor="#22c55e" trailColor="#374151" className="rounded-full"/>
              ...
              <th>Practice</th>
             <th >Revision</th>
             <th className='rounded-tr-xl'>Difficulty</th>
             </tr>
             </thead>
         <tbody>
        ...
        <tr key={lectureindex} className="border h-15 border-gray-700 text-white">
          ...
          {/* lecture Title */}
          <td className='text-center'>{lecture.lectureTitle}</td>
          {/* lecture resources */}
         <td className="py-3 px-4 align-middle">
          <div className="flex justify-center items-center">
          <FaYoutube onClick={()=>setplayerData({videoId: lecture.lectureUrl.split("/").pop().split("?")[0],})}size={25} className="text-red-500 cursor-pointer"/>
          </div>
           </td>
           {/* practice */}
           <td>
            <Link className='flex justify-center items-center' to={lecture.practiceUrl}>
           <svg className='w-7' xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" viewBox="0 0 24 24" id="leetcode">
           ...
           <td>
            ...
            {/* Dificulty */}
            <td className='text-center'>{lecture.Dificulty}</td>
          </tr>
            ))}
            </tbody>
             </table>
  ...
  <div className="flex justify-center w-full relative bottom-50">
    <div className="relative border py-14 px-7 max-w-2xl shadow-2xl z-10 rounded-t md:rounded-none bg-zinc-900">
      ...
      <button
        onClick={() => setplayerData(null)} className="absolute top-2 right-2 bg-red-500 text-white py-1 px-3 rounded cursor-pointer hover:bg-red-600">
        Cancel
      </button>
      <button onClick={()=>navigate(`/player/${id}`)} className='absolute py-2 px-3 rounded bg-orange-500 cursor-pointer text-white hover:bg-orange-600 top-1 right-22'>Open in New Tab</button>
      {/* YouTube video */}
      <YouTube
